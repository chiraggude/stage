...
(function($) {
  ...
  var S = {
    ...
    // Populate an ordered list from the section array we built
      ...
      var $headline = $('<span />', {'class': 'scroll-nav__heading wpbs-scrollnav-heading', text: S.settings.headlineText});
      var $wrapper  = $('<div />', {'class': 'scroll-nav__wrapper '});
      var $nav      = $('<div />', {'class': 'scroll-nav', 'role': 'navigation'});
      var $nav_list = $('<ul />', {'class': 'scroll-nav__list list-group'});

      $.each(sections, function(i) {
        var $item     = (i === 0) ? $('<li />', {'class': 'scroll-nav__item active list-group-item'}) : $('<li />', {'class': 'scroll-nav__item list-group-item'});
        var $link     = $('<a />', {'href': '#' + this.id, 'class': 'scroll-nav__link wpbs-muted', text: this.text});
        var $sub_nav_list;
